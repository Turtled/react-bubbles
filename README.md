...
- [ ] Explain what a token is used for.
  - A token is used for authentication. A token is generated by the server and sent to authorized clients. Then, the clients can send this token to the server while requesting data/sending data and the server will remember making that token, and allow the request.

- [ ] What steps can you take in your web apps to keep your data secure?
  - A login system (client side authentication in general) that makes sure the client who wants to get/post/put/delete actually should be allowed to. In React we do this with tokens, PrivateRoutes, and a custom Axios implementation which requires a token before sending requests.

- [ ] Describe how web servers work.
  - A web server recieves get/post/put/delete requests from clients, does some processing, and then it could return some data back to the client. 
